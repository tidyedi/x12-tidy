...
def _wrap(line: bytes, element_separator: bytes) -> bytes:
    """A whole interchange around a reconstructed ISA line, so the reconstruction
    can be fed back through the front of the pipeline."""
    s = element_separator
    trailer = (
...
def test_terminator_is_normalised_to_tilde() -> None:
