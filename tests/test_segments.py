...
from x12_tidy.envelope.structure import drop_null_rows, split_segments
...
# --------------------------------------------------------------------------
# drop_null_rows -- the next step
# --------------------------------------------------------------------------


def test_drop_null_rows_removes_the_empty_pieces() -> None:
    trailer = b"GS*PO*A*B*20240101*1200*1*X*004010~ST*850*0001~~SE*2*0001~GE*1*1~IEA*1*000000001~"
    segments = split_segments(build_isa(trailer=trailer))
    assert drop_null_rows(segments) == [
        b"GS*PO*A*B*20240101*1200*1*X*004010",
        b"ST*850*0001",
        b"SE*2*0001",
        b"GE*1*1",
...
def test_drop_null_rows_leaves_a_clean_list_unchanged() -> None:
    assert drop_null_rows(_CLEAN_SEGMENTS) == _CLEAN_SEGMENTS


def test_drop_null_rows_on_empty_list() -> None:
    assert drop_null_rows([]) == []
