...
| Document | What it is |
| --- | --- |
| **[The x12-tidy Method](https://docs.tidyedi.com/the-x12-tidy-method.html)** | The one idea the step notes hang off — earn the four delimiters from structure first, then everything downstream is ordinary parsing plus repair. Read this first. [Markdown source](the-x12-tidy-method.md). |
...
| **[Reconstructing the ISA Line](https://docs.tidyedi.com/reconstructing-the-isa-line.html)** | Engineering note on Step 2, slice 2 — once the delimiters are trusted, rebuilding the canonical 105-byte line, and why that is a total function. [Markdown source](reconstructing-the-isa-line.md). |
