...
- **shorter than its width** → space-pad on the right — *except* **ISA13**
  ...
  space-pads on the **left**: right-justified, matching the fixed-width
  convention for a numeric field rather than the left-justified convention
  for everything else in the ISA segment. The fill is always a space, never
  a digit — padding with invented zeros would assert a value beyond what the
  sender actually sent. Downstream code trims the padding when it reads
  ISA13 instead of reconstruction fabricating content to avoid needing to.
  `isa.element-width`.
...
the terminator out entirely, where there is no byte to keep. A final assertion checks the length is 105 -- it cannot fail (sixteen
fixed-width elements, "ISA", and the separators sum to 105 by construction),
so it is a tripwire for a future change to the reconstruction, not a
reachable outcome.
